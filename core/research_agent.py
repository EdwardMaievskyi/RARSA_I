...
class ResearchAgent:
    ...
    def __init__(self):
        ...
        logger.debug(f"ResearchAgent initialized with max_iterations={self.max_iterations}")

    def process_query(self,
                      query: str,
                      progress_callback: Optional[Callable[[str], None]] = None) -> Dict[str, Any]:
        """Process a research query and return results."""
        logger.debug(f"process_query called with query length: {len(query) if query else 0}")
        ...
        try:
            ...
            logger.debug(f"Starting graph stream with max_iterations={self.max_iterations}")

            for event_count, event in enumerate(search_agent_graph.stream(
                initial_state,
                {"recursion_limit": self.max_iterations * 2 + 5}
            )):
                logger.debug(f"Processing event {event_count + 1}, current_node: {list(event.keys())[0]}")
                ...
                if current_node in terminal_nodes:
                    ...
                    if 'final_answer' in final_event_data and final_event_data['final_answer'] is not None:
                        logger.info("Final answer found, breaking from event loop")
                        break
            ...
            if final_event_data and final_event_data.get('final_answer'):
                ...
                if hasattr(final_summary, 'sources') and final_summary.sources:
                    logger.debug(f"Processing {len(final_summary.sources)} sources")
                ...
                logger.info(f"Query processed successfully with {len(sources_list)} sources")
                return {
                    "success": True,
